...
async function connect() {
  // Check for Node before importing from it. A browser resolves
  // "node:worker_threads" as a URL, fetches it, and logs a CORS failure --
  // harmless but noisy, and it shows up in any page that imports this module
  // for its helpers.
  let parentPort = null;
  ...
  if (scope != null && globalThis instanceof scope) {
    return {
      ...
      // addEventListener, not `onmessage =`: importing this module for its
      // helpers must not steal the message handler from whoever imported it.
      onMessage: (handler) => {
