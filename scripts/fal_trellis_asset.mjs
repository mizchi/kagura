...
async function preprocessImageIfRequested(args) {
  ...
  await execFileAsync("python3", [
    resolve("scripts/fal_trellis_preprocess.py"),
    "--image",
    args.image,
    "--output",
    outputPath,
...
async function main() {
  ...
  const queryAssetPath = toHacknslashQueryAssetPath(outputPath);
  ...
  console.log(
    `[fal] hacknslash_3d url http://127.0.0.1:8080/?generated_prop=${encodeURIComponent(queryAssetPath)}`,
  );
}
