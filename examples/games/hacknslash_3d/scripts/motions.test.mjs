...
test("checked-in HY motion sources reproduce the game clips offline", async () => {
  ...
  const generated = code(
    await readFile(new URL("../app/enemy_motion_generated.mbt", root), "utf8"),
  );
  ...
  for (const entry of manifest.clips) {
    ...
    assert.ok(
      generated.includes(code(emitMoonBitClip(result))),
      `${entry.name}: rebuild MoonBit with just hunter-motions-build`,
    );
    assert.ok(
      result.rootY.every((y) => Number.isFinite(y) && Math.abs(y) < 0.35),
    );
  }
});
...
test("bowstring draws back and the nocked arrow disappears on the gameplay release", async () => {
  ...
  assert.ok(
    string.values[23 * 3 + 2] < -0.3,
    "string is pulled behind the grip",
  );
...
test("hunter HY clips use the hunter rig and its own hand and bow sockets", async () => {
  const manifest = await json("hunter-manifest.json");
  const generated = code(
    await readFile(new URL("../app/hunter_motion_generated.mbt", root), "utf8"),
  );
  assert.equal(manifest.profile.joints.length, 17);
  for (const entry of manifest.clips) {
    const source = restoreMotionSource(
      await json(`source/${entry.source}/source-motion.json`),
    );
    const clip = bakeGameMotion(
      source,
      source.animations[0],
      manifest.profile,
      entry.options,
    );
    assert.ok(generated.includes(code(emitMoonBitClip(clip))));
    if (entry.source === "bow_shot") {
      const [string, arrow] = clip.extraChannels;
      assert.equal(string.joint, 15);
      assert.equal(arrow.joint, 16);
      assert.deepEqual(arrow.values.slice(23 * 3, 24 * 3), [1, 1, 1]);
      assert.deepEqual(arrow.values.slice(24 * 3, 25 * 3), [0, 0, 0]);
    }
  }
