...
test.describe("runtime smoke native", () => {
  ...
  test("runtime_smoke --target native", () => {
    const result = runMoon(["run", ".", "--target", "native"], `${process.cwd()}/examples/smoke/runtime_smoke`);
  ...
  test("runtime_smoke_native hook_font_load", () => {
    const result = runMoon(["run", ".", "--target", "native"], `${process.cwd()}/examples/smoke/runtime_smoke_native`);
    const output = `${result.stdout}\n${result.stderr}`;
    expect(result.status, output).toBe(0);
