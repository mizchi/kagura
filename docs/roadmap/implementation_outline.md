...
- `engine/kagura_engine/src/gfx_wgpu_native`
  - `backend.mbt`: `~/Downloads/wgpu` 由来の backend 実装
- `platform/js_runtime/src/gfx_webgpu` / `src/gfx_webgl` (planned)
  - ブラウザ backend 実装
- `engine/kagura_engine/src/asset`
  - `contracts.mbt`: image/shader/material/atlas 管理契約
- `engine/kagura_engine/src/text`
