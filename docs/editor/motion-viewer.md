...
2. **Project → motions/enemies.kgrmotion**（敵）または **motions/hunter.kgrmotion**（プレーヤー）、**motions/wolf.kgrmotion**（狼）をクリックする。
3. **素体**と**武器・モーションセット**を選ぶ。武器選択時はその武器の既定クリップになる。
4. 素体を変更しても、武器・モーション・再生位置は維持される。

ASHEN REALMS のゴブリン、コボルト、スケルトン、重装ゴブリン、ボスの5素体と、
拳の装具＋パンチ、槍＋刺突、触媒＋詠唱、弓＋射撃の4セットを同梱する。
例えばゴブリンに弓、スケルトンに槍を装着できる。射手と術師は同じスケルトン素体を共有する。
プレーヤーは共通の身体に人間・ゴブリン・コボルト・スケルトンの頭部を組み合わせた4素体と、
人間の軽装・革鎧・鉄鎧・呪晶装備の比較用モデルを選べる。各モデルに、鉈＋斬撃・槍＋刺突・拳具＋パンチ・触媒＋魔法弾・弓＋射撃の5セットを同梱する。
...
狼は15関節の四足骨格を持つ独立アセット。「牙・四肢」セットから歩行・走行・噛み付きを選ぶ。
噛み付きの Impact は34フレーム目で、顎を閉じるタイミングとゲームの命中判定が一致する。
...
`CharacterModel` は17関節の骨格を共有し、`app/character_assets.mbt` が素体・種族頭部・部位別装備を組み合わせる。
`app/character_outfit.mbt` は装備状態から外見を導出する。頭部は敵の種族モデルと同じ造形を使い、
エンジンの `RigidGeometry.append_joint` で頭部の剛体三角形だけを抽出してリグへ接続する。
装備部品のGPUバッファは共有し、組み合わせの検査用メッシュのキャッシュは16件を上限にする。
