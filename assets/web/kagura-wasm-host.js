...
/** Guest stdout goes to the console in a browser, to stdout under Node. */
const defaultWrite = (text) => {
  if (globalThis.process?.stdout?.write != null) globalThis.process.stdout.write(text);
 ...
 *
 * This is the default strategy, used when nothing else drives the guest. The
 * worker host swaps in a wait that also wakes on a frame tick -- see
 * `kagura-wasm-worker.js`.
 */
...
/**
 * Build the import object for a guest module.
 *
 ...
 * @param {(ms: number) => void} [options.wait] how the event loop blocks. The
 *   default just sleeps; the worker host passes one that also wakes on a frame
 *   tick, which is what lets a main-thread rAF drive the guest.
 * @param {() => number} [options.frameNumber] backs `kagura_web.frame_number`
 * @returns {{imports: object, bind: (i: WebAssembly.Instance) => void, stats: object}}
 */
export function createWasmHost(bytes, options = {}) {
  const write = options.write ?? defaultWrite;
  const maxWaitMs = options.maxWaitMs ?? 50;
  const sleep = options.wait ?? makeBlockingSleep();
  ...
  imports.kagura_web = {
    ...(imports.kagura_web ?? {}),
    now_ms: () => Date.now(),
    // How many frames the host has driven so far. Stays 0 when nothing drives
    // the guest, so the same binary runs with and without a frame source.
  ...
  if (wasi != null) {
    wasi.fd_write = (_fd, iovs, iovsLen, nwritten) => {
      ...
      for (let i = 0; i < iovsLen; i++) {
        ...
        text += DECODER.decode(new Uint8Array(memory.buffer, ptr, len).slice());
        total += len;
      }
      write(text);
  ...
  return { imports, bind: (instance) => (memory = instance.exports.memory), stats };
}
 ...
 */
export function runWasm(bytes, options = {}) {
  const chunks = [];
  const host = createWasmHost(bytes, { ...options, write: (text) => chunks.push(text) });
