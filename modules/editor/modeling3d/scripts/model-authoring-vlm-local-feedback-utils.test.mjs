...
test("buildStampReviewMarkdown summarizes manual sculpt stamp issues", () => {
  const markdown = buildStampReviewMarkdown({
    targetFile: "modules/editor/modeling3d/examples/frog_authoring/src/model_doc.mbt",
    patchPayload: {
  ...
  assert.match(markdown, /Target file: `modules\/editor\/modeling3d\/examples\/frog_authoring\/src\/model_doc\.mbt`/);
...
test("buildStampReviewMarkdown reports when there are no sculpt stamp issues", () => {
  const markdown = buildStampReviewMarkdown({
    ...
    review: {
      target_file: "modules/editor/modeling3d/examples/chair_authoring/src/model_doc.mbt",
    },
  });
