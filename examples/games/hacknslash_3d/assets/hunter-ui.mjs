...
import {createTerrainInput,renderTerrainPanel,bindTerrainPanel} from './hunter-terrain.mjs';
import {createCameraInput,renderCameraPanel,bindCameraPanel,bindTouchCamera,syncCameraViewport} from './hunter-camera.mjs';

const paths={
  skull:'M8 24V20C-1 10 7 3 17 3s18 7 9 17v4h-3v7H11v-7ZM10 13h4v4h-4ZM20 13h4v4h-4ZM15 23l2-4 2 4M15 26v5M19 26v5',
  zombie:'M11 12V7q6-8 12 0v5l-4 3H15ZM15 16l-3 9-5 5M19 16l3 9 5 5M12 18 4 13 2 6M22 18l8-5 2-7M14 8h2M20 8h2',
  chest:'M4 15v14h26V15ZM4 15V9l4-5h18l4 5v6M17 15v7M8 15V6M26 15V6',
...
let current=null;
const cameraInput=createCameraInput({save:value=>{try{localStorage.setItem('ashen-hunt-camera-v1',value);}catch{}}});
globalThis.__ashenCamera=cameraInput;
const terrainInput=createTerrainInput();
globalThis.__ashenTerrain=terrainInput;
window.addEventListener('pagehide',()=>cameraInput.flush());
document.addEventListener('visibilitychange',()=>{if(document.hidden)cameraInput.flush();});
bindTouchCamera(document.querySelector('#app'),cameraInput,()=>current?.camera?.mode===1&&current.mode==='playing'&&!current.paused&&current.menu==='none'&&!current.arts?.targeting);
bindHunterAim({stage:document.querySelector('#app'),hud:root,input,
  enabled:()=>current?.mode==='playing'&&!current.paused&&current.menu==='none',tps:()=>current?.camera?.mode===1});

bindHunterMouse({stage:document.querySelector('#app'),hud:root,input,view:()=>current});

let panelKey='';
let showTreeDetail=false;
const names=['通常攻撃','ワールウィンド','突進斬り','盾ガード'];
root.innerHTML=`
  ${gamepadGuide}
  ...
  <button id="chest-prompt" class="waypoint-prompt" data-key="66" aria-label="宝箱を開ける" hidden>${icon('chest')}<span>宝箱を開ける</span><kbd>B</kbd></button>
...
$('hunter-panel').addEventListener('change',e=>{
  ...
  if(e.target.hasAttribute('data-hunter-slot'))input.tap(200+Number(e.target.dataset.hunterSlot),Number(e.target.value));
});
...
function activate(button,e){
  ...
  if(button.dataset.hold){if(button.dataset.hold!=='whirlwind')input.tap(hunterHeldActionKey(button.dataset.hold));input.hold(e.pointerId,button.dataset.hold);button.setPointerCapture(e.pointerId);button.dataset.pressed='';}
...
root.addEventListener('click',e=>{
  ...
  input.tap(b.dataset.hold?(hunterHeldActionKey(b.dataset.hold)):Number(b.dataset.key),Number(b.dataset.selection??-1));
});
// Keyboard activation of the focused channel button also follows hold/release.
root.addEventListener('keydown',e=>{
  if(!['whirlwind','skull'].includes(e.target?.dataset?.hold)||!['Space','Enter'].includes(e.code))return;
  e.preventDefault();e.stopPropagation();
  if(!e.repeat&&!e.target.disabled)input.hold(-51,e.target.dataset.hold);
});
...
window.addEventListener('keyup',e=>{
  const slot=hunterDigitSlot(e.code);
  if(slot>=0){input.release(-2100-slot);e.preventDefault();}
});
...
window.addEventListener('keydown',e=>{
  ...
  if(slotIndex>=0&&current?.mode==='playing'&&!current.paused&&current.menu==='none'){
    e.preventDefault();e.stopImmediatePropagation();
    if(!e.repeat){
      pressHunterSlot(input,current,slotIndex,-2100-slotIndex);
    }
    return;
  }
...
function panel(hud){
  ...
  el.classList.toggle('inventory-open',state==='inventory');
  el.classList.toggle('camera-open',state==='camera'||state==='terrain');
  document.body.classList.toggle('camera-editor-open',state==='camera'||state==='terrain');
  root.dataset.blocked=state?'true':'false';
  const changed=state!==previousPanelState;
  previousPanelState=state;
  if(changed){cameraInput.clear();terrainInput.clear();}
  if(state!=='inventory')inventoryPanel.close();
  if(!state){
    syncCameraViewport(el,false);
...
let previousHp=null;
const damageLabels=[];
const renderSummons=createSummonStatus($('summon-status'),icon);
const renderLoot=createLootLabels($('ground-loot'),{input,enabled:()=>current?.mode==='playing'&&!current.paused&&current.menu==='none'&&!current.arts?.targeting&&!current.camera?.editing&&!current.terrain?.editing});
...
function render(hud){
  ...
  const exploration=hud.exploration;
  const canOpenChest=exploration?.nearby_chest>=0&&!hud.arts?.targeting&&hud.mode==='playing'&&!hud.paused&&hud.menu==='none';
  $('chest-prompt').hidden=!canOpenChest;
  $('site-label').hidden=!exploration?.site_name;
  setText('site-label',exploration?.site_name??'');
  if(exploration){
    root.querySelector('.map-road').setAttribute('d',exploration.road_path);
  ...
  hud.skills.forEach((skill,i)=>{
    ...
    if(b.dataset.action!==String(skill.key)){
      ...
      if(skill.hold){b.dataset.hold=skill.hold;delete b.dataset.key;}
    ...
    const holding=whirling||(skill.hold==='guard'&&hud.arts?.guarding)||(skill.hold==='attack'&&hud.attack_remaining>0)||(skill.hold==='skull'&&input.isHeld('skull'));
    b.disabled=(!holding&&(status.disabled||hud.weapon_locked)) || hud.arts?.targeting || hud.paused || hud.menu!=='none';
...
const gamepad=createHunterGamepad({reader:createGamepadReader(),input,camera:cameraInput,view:()=>current,
  inventory:(action,value)=>inventoryPanel.handleGamepad(action,value),
