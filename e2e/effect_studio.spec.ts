...
test("effect_studio publishes AI patch bundle context", async ({ page }) => {
  ...
  expect(context.parsed?.patch_bundle.target_file).toBe("modules/editor/effect-studio/src/effect_doc.mbt");
...
test("effect_studio downloads bundle JSON and review markdown", async ({ page }) => {
  ...
  expect(bundleText).toContain("\"target_file\":\"modules/editor/effect-studio/src/effect_doc.mbt\"");
