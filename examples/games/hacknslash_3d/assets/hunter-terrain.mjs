...
export function createTerrainInput(){
  let pending=[];
  return {
    generate(values){if((values.length===5||values.length===6)&&values.every(Number.isFinite))pending=[...values];},
...
export function renderTerrainPanel(view){
  ...
  const exploration=view.pattern===5;
  const relief=Math.round((view.plains_relief??1)*100);
  return `<button class="close-panel" data-key="27" aria-label="一時停止メニューに戻る">←</button>
    ...
    <div class="camera-sliders terrain-sliders">${exploration?`<label><span>平地の起伏<output data-terrain-output="5">${relief}%</output></span><input type="range" aria-label="平地の起伏" data-terrain-field="5" min="0" max="100" step="5" value="${relief}"></label><p class="camera-help">0%で細かな凹凸をなくし、100%で従来の起伏。高台・街道・川・橋・洞窟は残ります。</p>`:''}${fields.map(([id,key,label,min,max,step])=>exploration?`<input type="hidden" data-terrain-field="${id}" value="${view[key]}">`:`<label><span>${label}<output data-terrain-output="${id}">${view[key]}</output></span><input type="range" aria-label="${label}" data-terrain-field="${id}" min="${min}" max="${max}" step="${step}" value="${view[key]}" ${view.pattern===0||(id===4&&view.pattern===3)?'disabled':''}></label>`).join('')}</div>
    <div class="terrain-actions"><button data-terrain-apply>${exploration?'起伏を反映':'生成して比較'}</button>${exploration?'<button data-terrain-gentle>なだらか · 35%</button>':'<button data-terrain-next>次のシード</button>'}</div>
    ...
    <p class="camera-help">${exploration?'平地の起伏は現在のセーブに保存します。近景・遠景・当たり判定へまとめて反映します。':'他の方式は比較用です。全方式を同じ三角形で描画。方式ボタンで即比較、数値は「生成して比較」で反映します。'}</p>
...
export function bindTerrainPanel(root,input){
  root.addEventListener('input',event=>{
    ...
    if(output)output.textContent=event.target.value+(field==='5'?'%':'');
  });
  root.addEventListener('click',event=>{
    const button=event.target.closest('[data-terrain-pattern],[data-terrain-apply],[data-terrain-next],[data-terrain-gentle]');
    if(!button)return;
    ...
    const relief=root.querySelector('[data-terrain-field="5"]');
    if(pattern===5&&relief)values.push(button.hasAttribute('data-terrain-gentle') ? .35 : Number(relief.value)/100);
