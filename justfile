...
test:
    ...
    for dir in examples/*/ tools/modeling3d/examples/*/ tools/effect-studio/examples/*/; do if [ -f "$dir/moon.mod.json" ]; then if [ "{{target}}" = "native" ] && { [ "$dir" = "examples/runtime_smoke_native/" ] || [ "$dir" = "examples/arena3d/" ] || [ "$dir" = "tools/effect-studio/examples/effect_studio/" ]; }; then echo "skip $dir (native test is validated separately or limited to check-only)"; continue; fi; (cd "$dir" && if [ "{{target}}" = "native" ]; then CPATH="$(brew --prefix glfw)/include:${CPATH:-}" LIBRARY_PATH="$(brew --prefix)/lib:${LIBRARY_PATH:-}" moon test --target native; else moon test --target {{target}}; fi) || exit 1; fi; done
