...
### 2.5 `engine/ui` は必要な意味情報を持っているが公開していない

| 持っているもの | 場所 |
|---|---|
| ノードごとの矩形 `LayoutRect{x,y,width,height}` + `UINodeId` | `engine/ui/contracts.mbt:31,44` |
| 座標→ノードの hit test（`hit_test` / `hit_test_all`） | `engine/ui/contracts.mbt:109,125` |
| フォーカス順（`UIFocusManager::focus_next/prev`, `set_focusable`） | `engine/ui/contracts.mbt:151-249` |
| テキスト実測（`TextRenderer::measure`） | `engine/text/contracts.mbt:166` |
| クリップ矩形とその交差（`ScissorRect::intersect` / `ScissorStack`） | `engine/widget2d/scissor.mbt:27,54` |
...
`editor/modeling3d/scripts/` に screenshot + context JSON + prompt → 構造化 review →
...
これは `engine/ui` の `LayoutResult{node_id, rect}` とほぼ 1:1 で写せる。
...
`engine/ui` の情報を 1 本の JSON として JS / native の両方に publish する。
