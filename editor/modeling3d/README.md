...
```
editor/modeling3d/
  moon.mod                 mizchi/kagura_modeling3d — 共有ライブラリ
  ...
  examples/<name>_authoring/
    model_doc.mbt          この example のモデルデータのみ
    authoring_profile.mbt  この example のレビュー方針
    main.mbt               run_authoring を呼ぶだけ（js）
    main_native.mbt        run_authoring_native を呼ぶだけ（native）
    main_stub.mbt          その他 target 用スタブ
...
- todo: `editor/modeling3d/TODO.md`

## example を追加するとき

必要なのは 4 ファイルだけです。エディタ・レンダラ・round-trip loop は共有ライブラリ側にあります。

1. `model_doc.mbt` — `default_model_document()` と `apply_imported_patch*()`
   ...
   - **`fn default_model_document() -> ModelDocument {` と
     ...
     この文字列を探してパッチを当てる
2. `authoring_profile.mbt` — `review_profile_id` / `review_constraints` / `doc_source_path`
   - この 3 つが VLM のレビュープロンプトとバンドルにそのまま入る
3. `main.mbt` / `main_native.mbt` / `main_stub.mbt` — 各 3 行
4. `moon.mod` に `"mizchi/kagura_modeling3d@0.1.0"`、`moon.work` に `"../.."` を追加

## 共有コードを直すとき

共有ライブラリを 1 箇所直せば 5 example に反映されます（以前は 11 ファイルが 5 重複していて、
`main.mbt` の 1 行修正に 5 箇所の編集が必要でした）。

```sh
cd editor/modeling3d
