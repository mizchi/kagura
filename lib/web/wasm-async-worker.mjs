...
import { createAsyncWasmHost } from "./wasm-async-host.mjs";
 ...
 */
export function makeFrameAwareWait(control, { maxWaitMs = 50 } = {}) {
  return (ms) => {
    if (ms <= 0) return;
 ...
 */
export function runGuest(bytes, control, options = {}) {
  const chunks = [];
  const module = new WebAssembly.Module(bytes);
  const host = createAsyncWasmHost(module, bytes, {
    ...options,
    write: (text) => chunks.push(text),
    wait: makeFrameAwareWait(control, options),
...
link?.onMessage((message) => {
  ...
  try {
    const { output, stats } = runGuest(new Uint8Array(message.wasm), control, message.options ?? {});
