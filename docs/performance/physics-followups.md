...
今回の成果（`step_pile_256`、main と交互 5 回の中央値）:

| | 前 | 後 | 倍率 |
|---|---:|---:|---:|
| physics2d | 5.26 ms | **1.29 ms** | **4.08x** |
| physics3d（起きている pile） | 3.67 ms | 3.14 ms | 1.17x（1024 体では 1.12x で分離）|
| physics3d（寝ているシーン） | 1.48 ms | 845 µs | **1.75x** |

**3D と 2D の絶対値を跨いで比較しないこと。** このコンテナは日をまたいで 1.4 倍ほど
速度が変わる（1 周目の doc の 4.72 ms と今回の 3.67 ms は同じコードである）。だから
上の表はすべて**同一 run 内のペア**であり、1 周目の doc の数字と直接は比べられない。

同一 run で並べた最終状態は 2D が 1.25 ms、3D が 3.32 ms。**physics2d は着手前は
physics3d より遅かった**（同一 run で 5.26 対 3.14 ms）ので、2D-first のエンジンで
逆になっていたのが正しい順序に戻ったことになる。
