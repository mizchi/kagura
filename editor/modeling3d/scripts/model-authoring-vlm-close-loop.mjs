...
function usage() {
  console.error(
    [
      "Usage: node editor/modeling3d/scripts/model-authoring-vlm-close-loop.mjs \\",
...
try {
  ...
  const initial = runNodeJson(
    "editor/modeling3d/scripts/model-authoring-vlm-handoff.mjs",
    plan.initialArgs,
    plan.cwd,
  );
  ...
  const feedback = runNodeJson(
    "editor/modeling3d/scripts/model-authoring-vlm-local-feedback.mjs",
    [
      "--bundle",
      initial.bundle,
      "--parsed",
  ...
  const apply = shouldApplyBundlePatch(initialSummary)
    ? runNodeJson(
        "editor/modeling3d/scripts/model-authoring-vlm-apply-patch.mjs",
        [
          "--target",
