...
export function renderWorldMap(hud,{icon,escape}) {
  ...
  const canTravel=atlas.near_waypoint&&selected.unlocked&&selected.id!==atlas.current&&!travelling&&!hud.weapon_locked;
  ...
  return `<p class="eyebrow">THE OLD PILGRIM ROAD</p><h2>世界地図</h2>
    ...
    <div class="atlas-layout">
      ...
      <aside class="atlas-detail" aria-label="選択したウェイポイント">
        ...
        <button class="atlas-travel" data-key="13" data-selection="${selected.id}" data-focus="world-travel" ${canTravel?'':'disabled'}>${travelling?'灯火への道を準備中…':selected.id===atlas.current?'このエリアにいます':!selected.unlocked?'ウェイポイント未解放':!atlas.near_waypoint?'灯火の近くで移動できます':hud.weapon_locked?'動作終了後に移動できます':'この灯火へジャンプ'}<span>${travelling?'◌':'→'}</span></button>
    ...
    </div><p class="atlas-footer">${atlas.near_waypoint?'✧ 灯火のそばにいます。解放済みの地点へ移動できます。':'灯火のそばから、解放済みの地点へ移動できます。'}<span>G / ESC で戻る</span></p>`;
}
