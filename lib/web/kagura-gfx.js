...
function getOrCreateCustomPipeline(gpu, device, cmd, format) {
  ...
  const key = `${cmd.shaderId}_${format}_${cmd.blendMode}_${samples}_${JSON.stringify(raster)}`;
    ...
    if (tb.type === "texture") {
      ...
      if (cmd.srcImageIds != null && srcIndex < cmd.srcImageIds.length) {
        ...
        if (imgId > 0 && gpu.textures != null) {
          ...
          if (texEntry != null) {
            texView = texEntry.view;
            textureSampler = texEntry.sampler ?? sampler;
      ...
      bgEntries.push({ binding: tb.binding, resource: textureSampler });
    }
  }
