...
authoring workspace は `modules/editor/modeling3d/` に集約してあり、example の実体は `modules/editor/modeling3d/examples/*` にある。

エディタ・レンダラ・export・round-trip review loop は共有ライブラリ
`mizchi/kagura_modeling3d`（`modules/editor/modeling3d/src/`）にあり、各 example が持つのは
モデルデータ（`model_doc.mbt`）とレビュー方針（`authoring_profile.mbt`）だけ。
構成の詳細は [`modules/editor/modeling3d/README.md`](../../modules/editor/modeling3d/README.md)。
