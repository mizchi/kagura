export const gamepadGuide=`<details id="gamepad-guide"><summary id="gamepad-status">ゲームパッド · 接続を確認</summary><div>
  ...
  <p>A / ×：決定<br>X / □：通常攻撃（長押し）<br>RB / R1：盾ガード（長押し）<br>B / ○：回避／取消</p>
  <p id="gamepad-primary-skills">Y / △：処刑の一撃<br>RT / R2：ワールウィンド（長押し）<br>LB / L1：分裂火弾<br>LT / L2 ＋ 十字 ↓：霜の輪</p>
  <p>習得後：<br>LT / L2 ＋ 十字 ↑：連鎖雷撃<br>LT / L2 ＋ 十字 ←：追尾の呪弾<br>LT / L2 ＋ 十字 →：反撃の構え</p>
  <p>十字 ↑：宝箱を開ける<br>十字 ←：武器切替<br>十字 ↓：星落とし<br>十字 →：踏込斬り<br>L3：突進 / R3：TPS切替</p>
...
export function createGamepadGuide(root){
  ...
  return {
    ...
    render(hud,active){
      ...
      text(root.querySelector('#gamepad-primary-skills'),['Y / △','RT / R2','LB / L1','LT / L2 ＋ 十字 ↓'].map((label,i)=>`${label}：${hud.skills?.[i]?.name??''}${hud.skills?.[i]?.hold==='whirlwind'?'（長押し）':''}`).join('\n'));
      for(const [i,label] of ['Y / △','RT / R2','LB / L1','LT / L2 + ↓'].entries())text(root.querySelector(`[data-skill="${i}"] kbd`),active?label:String(i+1));
      text(root.querySelector('#attack-button kbd'),active?'X / □':'J / 左クリック');
      if(hud.dodge<=0)text(root.querySelector('#dodge-status'),active?'B / ○':'SPACE');
      if(hud.charge.remaining<=0)text(root.querySelector('#charge-status'),active?'L3':'C');
