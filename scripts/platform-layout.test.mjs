...
test('platform contract and JS implementation stage as separate publishable modules', t => {
  ...
  const implementation = byName.get('mizchi/kagura_platform_js');
  assert.equal(contract.dir, 'platform');
  assert.equal(implementation?.dir, 'platform_js');
  ...
  const implementationDir = join(outDir, 'mizchi__kagura_platform_js');
  assert.ok(existsSync(join(contractDir, 'contracts.mbt')));
  assert.ok(existsSync(join(implementationDir, 'web_core', 'controls.mbt')));
  for (const nested of ['web_runtime_hooks', 'native_runtime_hooks', 'platform_js']) {
...
test('default boundary policy permits implementations to use the contract and rejects reverse or engine dependencies', t => {
  ...
  const modules = {
    ...
    platform_js: 'mizchi/kagura_platform_js',
  ...
  writeFileSync(join(repoRoot, 'platform_js/moon.pkg'), 'import { "mizchi/kagura_platform" @platform }\n');
  assert.deepEqual(check().errors, []);
  writeFileSync(join(repoRoot, 'platform/moon.pkg'), 'import { "mizchi/kagura_platform_js/web_core" }\n');
  assert.equal(check().violations.length, 1);
  writeFileSync(join(repoRoot, 'platform/moon.pkg'), '');
  writeFileSync(join(repoRoot, 'platform_js/moon.pkg'), 'import { "mizchi/kagura_engine/runtime" }\n');
  assert.equal(check().violations.length, 1);
});
