...
CPU変形の結果を同時に異なるポーズで保持する場合は別のassetを使う。
GPU用の不変バッファは異なるポーズのキャラクター間でも共有できる。
...
`@scene3d.BakedSkinningClip::new(skeleton, clip, frame_count)` は有限個の骨行列を初期化時に作る。
`frame_index(phase)` で位相を共有できる番号へ変換し、`matrices(index)` をGPUスキニングへ渡す。
位相0と1を含み、各行列は変更しない借用データとして扱う。ループの周回は呼び出し側で決める。
同じ骨格・クリップ・位相の敵をインスタンス描画すれば、敵ごとの骨計算とCPU頂点変形を省ける。
