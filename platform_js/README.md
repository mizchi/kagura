...
`platform_js` は JS 専用ランタイムを MoonBit で記述する module です。

- root module: `../moon.mod`
- js runtime module: `moon.mod`
- package prefix: `mizchi/kagura_platform_js/*`
...
実ブラウザでは `platform/web_runtime_hooks` が DOM・描画・音声を結線し、
...
| `inventory.mbt` | 形状回転、配置プレビュー、ドロップ領域、装備比較 |
...
- インベントリは既存の検証済み整数セルを使う読み取り専用プレビュー。確定はゲーム側。
...
ゲームの dev server と Studio の Vite は起動時と `web_core` 編集時に再生成します。
