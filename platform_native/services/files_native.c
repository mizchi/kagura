...
MOONBIT_FFI_EXPORT int kgr_capture_load(const unsigned char *path, int length, const unsigned char *env, int env_length) {
  ...
  name[length] = 0;
  char *env_name = malloc((size_t)env_length + 1);
  if (!env_name) { free(name); return -2; }
  memcpy(env_name, env, (size_t)env_length); env_name[env_length] = 0;
  const char *override = env_length ? getenv(env_name) : NULL;
  free(env_name);
