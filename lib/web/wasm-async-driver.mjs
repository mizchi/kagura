...
// they meet on a small shared Int32Array. See `wasm-async-worker.mjs` for the
// handshake.
//
...
// back in.
//
// Only the Node path is covered by tests; `createBrowserFrameSource` is the
// same handshake against `requestAnimationFrame`.

import { CONTROL_LENGTH, FRAME_SLOT, STOP_SLOT } from "./wasm-async-worker.mjs";

const WORKER_URL = new URL("./wasm-async-worker.mjs", import.meta.url);
...
export function requestStop(control) {
 ...
 * interval used in `runInWorker`; returns a stop function.
 */
export function createBrowserFrameSource(control) {
  let handle = 0;
  const step = () => {
    tickFrame(control);
 ...
 */
export async function runInWorker(wasmBytes, options = {}) {
